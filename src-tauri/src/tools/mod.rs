...
pub mod format {
    ...
    pub fn format_relations(relations: &[EntryRelation], current_entry_id: &str) -> String {
        ...
        for rel in relations {
            ...
            result.push_str(&format!(
                "- {} {} {}\n  关系描述：{}\n\n",
                if rel.a_id == current_entry_id { "我" } else { "对方" },
                direction,
                if rel.a_id == current_entry_id { "对方" } else { "我" },
                rel.content
            ));
        }
        result
...
/// 获取项目标签定义
pub async fn list_tag_schemas(state: &AppState, project_id: &str) -> Result<Vec<TagSchema>, String> {
    let db = state.sqlite_db.lock().await;
